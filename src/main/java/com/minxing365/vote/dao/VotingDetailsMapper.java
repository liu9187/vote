...
public interface VotingDetailsMapper {
     ...
     * @return
     */
    @Select("SELECT sa.`modify`,sa.`name`,si.user_id AS userId,si.app_id AS appId,si.forms  FROM survey_apps sa, survey_instance si WHERE sa.id = si.app_id AND sa.`name` =#{name} GROUP BY si.id; ")
